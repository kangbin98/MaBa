"""
baseline model -->clip from https://github.com/openai/CLIP
"""
...
logger = logging.getLogger("MaBa.model")
    ...
    def forward(self, x: torch.Tensor):
        """
                ...
                1. 卷积分块处理
                   ...
                   → (B, 768, 24, 8)
                   ※ 计算验证: (384-16)/16+1=24, (128-16)/16+1=8
                ...
                3. 维度置换
                   permute(0,2,1) → (B, 192, 768)

                4. 添加CLS Token
                   cat([cls_token, x], dim=1) → (B, 193, 768)
                   ※ cls_token: (1,1,768) → expand到(B,1,768)

                5. 位置编码
                   + positional_embedding → (B, 193, 768)
                   ※ positional_embedding: (193,768)

                6. Transformer处理
                   permute(1,0,2) → (193, B, 768)
                   → Transformer layers保持维度 → (193, B, 768)
                   permute(1,0,2) → (B, 193, 768)

                7. 层归一化+投影
                   LayerNorm → (B, 193, 768)
                   @ proj(768,512) → (B, 193, 512)
                """
