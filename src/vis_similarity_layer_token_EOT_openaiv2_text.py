...
import os
from model import build_model  # 确保baseline模型的build_model函数正确导入
...
def get_text_attn_weights_hook(name):
    ...
    def hook(module, input, output):
        # 获取 last_attn_weights 属性，需根据baseline模型调整
...
if __name__ == "__main__":
    ...
    output_dir = "vis_baseline_EOT_text_attention"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # 定义模型参数类（根据baseline模型的需求进行调整）
    class Args:
        ...
        vocab_size = 10000  # 根据baseline模型的词汇表大小调整
        cmt_depth = 2
        id_loss_weight = 1.0
        mlm_loss_weight = 1.0
    ...
    # 注册文本注意力钩子
    hook_outputs = {}
    layers_to_hook = list(range(len(model.base_model.transformer.resblocks)))  # 根据baseline模型调整
    ...
    # 确定EOT token的索引
    # 通常，EOT token位于最后一个非填充token的位置
    fusion_idx = num_text_tokens - 1
    print(f"EOT token index: {fusion_idx}")
    fusion_token = tokens[fusion_idx]
    print(f"EOT token: {fusion_token}")
